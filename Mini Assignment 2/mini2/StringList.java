...
//)0
/**
 * A StringList represents a specialized list of strings that supports a set of
 * operations called <em>map</em>, <em>filter</em>, <em>reduce</em>, and
 * <em>reduceToList</em>. The argument to each of these methods is an interface
 * type representing an operation to be performed on the list.
 */
public class StringList
	...
	/**
	 * Constructs a StringList from the given file, where each line of the file
	 * becomes one item in the StringList.
	 * 
	 * @param filename
	...
	/**
	 * Returns a new StringList consisting of just the strings that are chosen by
	 * the given Selector.
	 * 
	 * @param selector
	...
	public StringList filter(Selector selector)
		...
		for (String d : data)
			{
			boolean temp = selector.select(d);
			if (temp == true)
				{
				result.append(d);
				}
	...
	/**
	 * Combines all strings in this list into a single value of type T using the
	 * given Combiner object and the given initial value. If this StringList is
	 * empty, this method just returns the given initial value.
	 * 
	 * @param combiner
	...
	/**
	 * Returns a list of objects obtained by applying the given conversion to each
	 * string in this StringList.
	 * 
	 * @param converter
	 *            a Converter object for converting strings to type T
	 * @throws FileNotFoundException 
	 */
	public <T> ArrayList<T> mapToList(Converter<T> converter) throws FileNotFoundException
		{
		ArrayList<T> result = new ArrayList<T>();
		for (String d : data)
			{
			T temp = converter.convert(d);
			result.add(temp);
			}
		return result;
		}
	...
	/**
	 * Returns a single string representing the contents of this StringList based on
	 * the format of java.util.ArrayList.
	 * 
	...
	/**
	 * Writes the contents of this StringList to the given output stream, one line
	 * per item.
	 * 
	 * @param out
	 *            given output stream
