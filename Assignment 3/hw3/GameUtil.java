...
public class GameUtil {
	...
	public TilePosition generateRandomTilePosition(int[][] grid, Random rand, Direction lastMove) {
		...
		if (lastMove == Direction.UP) {
			while (changed == false) {
				int n = rand.nextInt(gl);
				if (grid[gl - 1][n] == 0) {
					changed = true;
					return new TilePosition(gl - 1, n, 0);}}}
		if (lastMove == Direction.DOWN) {
			while (changed == false) {
				int n = rand.nextInt(gl);
				if (grid[0][n] == 0) {
					changed = true;
					return new TilePosition(0, n, 0);}}}
		if (lastMove == Direction.RIGHT) {
			while (changed == false) {
				int n = rand.nextInt(gl);
				if (grid[n][0] == 0) {
					changed = true;
					return new TilePosition(n, 0, 0);}}}
		if (lastMove == Direction.LEFT) {
			while (changed == false) {
				int n = rand.nextInt(gl);
				if (grid[n][gl - 1] == 0) {
					changed = true;
					return new TilePosition(n, gl - 1, 0);}}}
		return null;
	}
	...
	public ArrayList<Move> shiftArray(int[] arr) {
		int al = arr.length;
		ArrayList<Move> moves = new ArrayList<Move>(al - 1);
		int x = 1; 
		while (x < al) {
			if (arr[x] != 0 && arr[x - 1] == 0) {
				arr[x - 1] = arr[x];
				arr[x] = 0;
				moves.add(new Move(x, x - 1, arr[x - 1]));}
			if (mergeValues(arr[x - 1], arr[x]) != 0) {
				arr[x - 1] = mergeValues(arr[x - 1], arr[x]);
				arr[x] = 0;
				moves.add(new Move(x, x - 1, arr[x], arr[x - 1], arr[x] + arr[x - 1]));}
			x ++;}
		return moves;
	}
}
