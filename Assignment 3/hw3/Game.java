...
public class Game {
	...
	/**
	 * grid is the board set up in a certian number of arrays.
	 */
	private int[][] grid;
	
	/**
	 * game is used for calling GameUtil easily and referencing the same instance of the current game.
	 */
	private GameUtil game;
	
	/**
	 * size is the dimensions of the board ex. 3 by 3, the size here would be 3.
	 */
	private int size;
	
	/**
	 * random is just a random value that changes.
	 */
	private Random random;
	
	/**
	 * tileValue is the value of the next tile that will be placed after the player makes a move.
	 */
	private int tileValue;
	
	/**
	 * recentMove just keeps track of what direction the player chose on the most recent turn.
	 */
	private Direction recentMove;
	
	/**
	 * checker is a boolean variable to check if the undo method is viable.
	 */
	private boolean checker = false;
	
	/**
	 * ret is a copy of the previous board in case the player wants to use the undo function
	 */
	private int[][] ret;
	
	/**
	 * this makes sure that the game does not try to make a copy of the board on the first turn because their wouldn't be a previous board to store.
	 */
	private int turn;
	...
	public Game(int givenSize, GameUtil givenConfig, Random givenRandom) {
		...
		game = givenConfig;
		random = givenRandom;
		grid = game.initializeNewGrid(size, random);
		tileValue = game.generateRandomTileValue(random);
	}
	...
	public void updateRowOrColumn(int[] arr, int rowOrColumn, Direction dir) {
		...
		if (dir == Direction.UP) {
			while (x <= alen) {
				setCell(x, rowOrColumn, arr[x]);
				x++;}}
		if (dir == Direction.DOWN) {
			while (x <= alen) {
				setCell(x, rowOrColumn, arr[alen - x]);
				x ++;}}
		if (dir == Direction.LEFT) {
			while (x <= alen) {
				setCell(rowOrColumn, x, arr[x]);
				x ++;}}
		if (dir == Direction.RIGHT) {
			while (x <= alen) {
				setCell(rowOrColumn, x, arr[alen - x]);
				x ++;}}}
	...
	public int getSize() {
	   ...
	   */
	public int getScore() {
		return game.calculateTotalScore(grid);
	}
	...
	public int[] copyRowOrColumn(int rowOrColumn, Direction dir) {
		...
		if (dir == Direction.UP) {
			while (rc <= aLen) {
				copied[rc] = getCell(rc, rowOrColumn);
				rc ++;}}
		if (dir == Direction.DOWN) {
			while (rc <= aLen) {
				copied[rc] = getCell(aLen - rc, rowOrColumn);
				rc ++;}}
		if (dir == Direction.LEFT) {
			while (rc <= aLen) {
				copied[rc] = getCell(rowOrColumn, rc);
				rc ++;}}
		if (dir == Direction.RIGHT) {
			while (rc <= aLen) {
				copied[rc] = getCell(rowOrColumn, aLen - rc);
				rc ++;}}
		return copied;}
	...
	public ArrayList<Move> shiftGrid(Direction dir) {
		if (turn == 1) {
			ret = game.copyGrid(grid);}
		recentMove = dir;
		ArrayList<Move> moves = new ArrayList<Move>();
		for (int x = 0; x < size; x++) {
			int[] temp = new int[size - 1];
			temp = copyRowOrColumn(x, dir);
			game.shiftArray(temp);
			updateRowOrColumn(temp, x, dir);
			moves.addAll(game.shiftArray(copyRowOrColumn(x, dir)));}
		TilePosition add = newTile();
		grid[add.getRow()][add.getCol()] = add.getValue();
		checker = true;
		turn = 1;
		return moves;}
	...
	public TilePosition newTile() {
		checker = false;
		TilePosition tileInfo = game.generateRandomTilePosition(grid, random, recentMove);
		tileInfo.setValue(tileValue);
		grid[tileInfo.getRow()][tileInfo.getCol()] = tileValue;
		getScore();
		return tileInfo;}
}
